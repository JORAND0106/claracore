...
COMMENT ON VIEW public.vista_so_registros_matriz_validacion IS
  'Filas con ítem asignado para dashboard_matriz_validacion_agg (incluye costo_directo canónico).';
...
base AS (
  SELECT
    ...
    COALESCE(
      r.costo_directo,
      round(COALESCE(r.cantidad_total, 0)::numeric * COALESCE(r.vlr_unitario, 0)::numeric, 0)
    )::numeric AS cd,
...
-- Una fila por (bloque, ítem, nivel activo): costo_directo neto en cada bucket de estado.
item_nivel AS (
  SELECT
    b.bloque,
    b.cap_k,
...
main_nivel AS (
  SELECT
    bloque,
    nivel,
    SUM(c_apr) AS aprobado,
    SUM(c_pend) AS pendiente,
    SUM(c_pend_item) AS pendiente_item,
    SUM(c_nr) AS no_revisado,
    SUM(c_rej) AS rechazado,
    SUM(c_hab) AS habilitado
  FROM item_nivel
...
COMMENT ON FUNCTION public.dashboard_matriz_validacion_agg(bigint, bigint) IS
  'Matriz validación SICOE: SUM(costo_directo) por bucket; prerrequisitos; claves nivel1..nivel6.';
