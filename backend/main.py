...
def _so_registro_validacion_audit_snapshot(row: Optional[dict]) -> Optional[dict]:
    ...
    keys = (
        ...
        "nivel4_estado", "nivel4_usuario_id", "nivel4_fecha",
        "nivel5_estado", "nivel5_usuario_id", "nivel5_fecha",
        "nivel6_estado", "nivel6_usuario_id", "nivel6_fecha",
...
CARGO_ID_NIVEL_MAP = {
    ...
    56: "nivel2_estado",  # Director de Obra (cadena contratista N2)
    10: "nivel3_estado",  # Director de Obra (validación N3)
    50: "nivel4_estado",  # Residente de Interventoría
    58: "nivel5_estado",  # Director de Interventoría
    9: "nivel5_estado",  # Director de Interventoría (id alterno)
}
...
CARGO_NIVEL_PRERREQUISITO = {
    ...
    "nivel4_estado": ("nivel3_estado", "Aprobado"),
    "nivel5_estado": ("nivel4_estado", "Aprobado"),
    "nivel6_estado": ("nivel5_estado", "Aprobado"),
}

# Filtros Sicoe y capas JSON: { "nivel": 1..6, "estado": "..." } o cargo_id.
...
def _get_nivel_numero_maximo_contrato(contrato_id: int) -> int:
    ns = _get_niveles_activos_contrato(contrato_id)
    return max(ns) if ns else 3


def _get_nivel_maximo_contrato(contrato_id: int) -> str:
    """Campo `nivelN_estado` del nivel más alto activo (p. ej. [1,2,4] → nivel4_estado)."""
...
def _matriz_estado_nivel_final_row(row: Optional[dict], contrato_id: int):
    if not row:
        return None
    fld = _get_nivel_maximo_contrato(contrato_id)
    return row.get(fld)


def _matriz_validacion_norm_estado_nivel_final(row: Optional[dict], contrato_id: int) -> str:
    return _matriz_validacion_norm_estado(_matriz_estado_nivel_final_row(row, contrato_id))
...
def _registro_nivel3_aprobado(row: Optional[Dict[str, Any]]) -> bool:
    ...
    if cid is not None:
        try:
            return _registro_nivel_max_aprobado(row, int(cid))
        except (TypeError, ValueError):
            pass
...
def _sicoe_capa_etiqueta_panel(capx: dict) -> str:
    ...
    if fld == "nivel4_estado":
        return NIVEL_VALIDACION_ENCABEZADO[4]
    if fld == "nivel5_estado":
        return NIVEL_VALIDACION_ENCABEZADO[5]
    if fld == "nivel6_estado":
        return NIVEL_VALIDACION_ENCABEZADO[6]
    return "Validación"
...
def _sicoe_nivel_num_desde_campo(campo: Optional[str]) -> Optional[int]:
    ...
    if campo == "nivel3_estado":
        return 3
    if campo == "nivel4_estado":
        return 4
    if campo == "nivel5_estado":
        return 5
    if campo == "nivel6_estado":
        return 6
    return None
...
def _sicoe_db_nivel_validacion_usuario(user_id: int) -> Optional[int]:
    ...
    if rn == "operativo contratista" or ("operativo" in rn and "contrat" in rn and "intervent" not in rn):
        return 1
    if "gerencial" in rn and "contrat" in rn and "intervent" not in rn:
        return 3
    if "gerencial" in rn and "intervent" in rn:
        return 5
    if "supervisor" in rn and ("externo" in rn or "entidad" in rn):
        return 6
    if rn == "contratista" or (
        "contrat" in rn
        and "intervent" not in rn
        and "gerencial" not in rn
        and "operativo" not in rn
        and "supervisor" not in rn
    ):
        return 2
    if "intervent" in rn:
        if cid in (9, 58):
...
NIVEL_FIELD_VALIDACION_AVANZADA = (
    "nivel2_estado",
    "nivel3_estado",
    "nivel4_estado",
    "nivel5_estado",
...
def get_presupuesto_panel_validacion_interv(
    contrato_id: int,
    nivel: str = Query("capitulo", description="capitulo | item"),
    capitulo: Optional[str] = None,
    capitulos: Optional[List[str]] = Query(None),
    item: Optional[str] = None,
    items: Optional[List[str]] = Query(None),
    ...
    """
    Agregado ligero para el panel de validación Interventoría (presupuesto vigente).
    Mismos filtros que GET /presupuesto; no devuelve filas individuales.
    Con nivel=item requiere capitulo (drill del panel).
    """
    _require_contract_access(current_user, contrato_id)
    ...
    out = fetch_panel_validacion_interv(
        supabase_execute,
        supabase,
        contrato_id,
        current_user,
        nivel=nv,
        capitulo=cap_drill if nv == "item" else None,
        tipo_ejecucion=tipo_ejecucion,
        filtros=filtros,
        orden_capitulos=None,
    )
    return out
...
def sincronizar_actas_rpo_por_vencimiento(contrato_id: int, current_user) -> dict:
    ...
    moved_total = 0
    cerrado_por_vencimiento = False
    acta_creada_auto = None

    vig = _acta_rpo_vigente_row(contrato_id)

    if not vig:
        rows = supabase.table("actas").select("*")\
            .eq("contrato_id", contrato_id).eq("tipo_grupo", "RPO")\
            .lt("fecha_fin", today_s)\
            .order("fecha_fin", desc=True).limit(1).execute().data or []
        if rows:
            L = rows[0]
            lid = int(L["id"])
            ff_L = (L.get("fecha_fin") or "")[:10]
            try:
                fc = date.fromisoformat(ff_L)
            except ValueError:
                fc = None
            if fc is not None and fc <= today:
                ...
                overlap_id = None
                for ex in existentes:
                    if int(ex.get("id") or 0) == lid:
                        continue
                    ei = (ex.get("fecha_inicio") or "")[:10]
                    ef = (ex.get("fecha_fin") or "")[:10]
                    if ei and ef and _acta_rpo_periodos_se_solapan(ei, ef, fi_ns, ff_ns):
                        overlap_id = int(ex["id"])
                        break
                if overlap_id:
                    moved_total += _mover_registros_residual_hacia_acta(contrato_id, [lid], overlap_id)
                else:
                    try:
                        r = _cerrar_acta_rpo_ejecutar(
                            contrato_id, fc, lid, L, current_user,
                            log_accion="cerrar_rpo_vencimiento_calendario",
                        )
                        cerrado_por_vencimiento = True
                        acta_creada_auto = r.get("acta_creada")
                        moved_total += int(r.get("registros_movidos_residual") or 0)
    ...
    out: Dict[str, Any] = {
        "ok": True,
        "vigente_id": vig["id"] if vig else None,
        "registros_movidos": moved_total,
        "cerrado_automatico_vencimiento": cerrado_por_vencimiento,
        "acta_creada_automatica": acta_creada_auto,
    }

    if not vig:
        out["sin_acta_vigente"] = True
        return out

    vig_id = int(vig["id"])
    past_rows = supabase.table("actas").select("id")\
        .eq("contrato_id", contrato_id).eq("tipo_grupo", "RPO")\
        .lt("fecha_fin", today_s).execute().data or []
    past_ids = [int(r["id"]) for r in past_rows if int(r["id"]) != vig_id]
    if past_ids:
        moved_total += _mover_registros_residual_hacia_acta(contrato_id, past_ids, vig_id)

    out["registros_movidos"] = moved_total
    return out


@app.post("/actas/{contrato_id}/rpo/sincronizar-vencimiento")
def actas_rpo_sincronizar_vencimiento(contrato_id: int, current_user=Depends(get_current_user)):
    """Alinea residuales por vencimiento de período (y crea acta siguiente si faltaba). Invocable desde panel admin."""
    return sincronizar_actas_rpo_por_vencimiento(contrato_id, current_user)
...
def mover_registro(contrato_id: int, registro_id: int, nuevo_reporte_id: int, current_user=Depends(get_current_user)):
    try:
        ...
        def _reg():
            return supabase.table("so_registros")\
                .select(f"reporte_id, contrato_id, {SICOE_SELECT_NIVELES_ESTADO}").eq("id", registro_id)\
                .eq("contrato_id", contrato_id).limit(1).execute().data
        ...
        if _registro_nivel3_aprobado(reg_rows[0]):
            raise HTTPException(
                status_code=400,
                detail="El registro está aprobado en el último nivel de validación: no puede moverse a otro reporte.",
            )
...
def _put_validar_nivel_sicoe_alto(
    contrato_id: int,
    registro_id: int,
    body: ValidarNivel3Body,
    nivel: int,
    current_user,
):
    """Lógica compartida de PUT validar-nivel3 … nivel6 (bloqueado solo al aprobar el nivel máximo activo del contrato)."""
    if nivel not in (3, 4, 5, 6):
        raise HTTPException(status_code=500, detail="Nivel interno inválido.")
    activos = _get_niveles_activos_contrato(contrato_id)
    if nivel not in activos:
        raise HTTPException(
            status_code=422,
    ...
    try:
        ...
        if (r0.get(campo) or "") == "Aprobado" and body.estado != "Aprobado":
            raise HTTPException(
                status_code=400,
                detail=f"El registro ya está aprobado en Nivel {nivel}. Use el flujo de reversión para modificar la validación.",
            )
        if r0.get(prev_f) != prev_ok:
            raise HTTPException(
                status_code=422,
        ...
        if body.comentario_data:
            ...
            _push_notif_validacion_sicoe_destinatarios(
                ...
                contrato_id,
                registro_id,
                f"Validación Nivel {nivel}: {body.estado}",
        ...
        try:
            ...
            registrar_log(
                ...
                "registro",
                str(registro_id),
                {"nivel": nivel, "estado": body.estado},
                valor_anterior=_so_registro_validacion_audit_snapshot(prev_audit),
                valor_nuevo=_so_registro_validacion_audit_snapshot(after_audit),
            )
        except Exception:
...
def validar_nivel3(contrato_id: int, registro_id: int, body: ValidarNivel3Body,
                   current_user=Depends(get_current_user)):
    return _put_validar_nivel_sicoe_alto(contrato_id, registro_id, body, 3, current_user)


@app.put("/sicoe-obra/{contrato_id}/registros/{registro_id}/validar-nivel4")
def validar_nivel4(contrato_id: int, registro_id: int, body: ValidarNivel3Body,
                   current_user=Depends(get_current_user)):
    return _put_validar_nivel_sicoe_alto(contrato_id, registro_id, body, 4, current_user)


@app.put("/sicoe-obra/{contrato_id}/registros/{registro_id}/validar-nivel5")
def validar_nivel5(contrato_id: int, registro_id: int, body: ValidarNivel3Body,
                   current_user=Depends(get_current_user)):
    return _put_validar_nivel_sicoe_alto(contrato_id, registro_id, body, 5, current_user)


@app.put("/sicoe-obra/{contrato_id}/registros/{registro_id}/validar-nivel6")
def validar_nivel6(contrato_id: int, registro_id: int, body: ValidarNivel3Body,
                   current_user=Depends(get_current_user)):
    ...
    try:
        ...
        if nivel == 1:
            ...
            for row in candidatos:
                ...
                _sicoe_aplicar_bloqueado_si_nivel_es_maximo(contrato_id, 3, update, marcar)
            ...
            activos_m = _get_niveles_activos_contrato(contrato_id)
            if nivel not in activos_m:
                raise HTTPException(
                    status_code=422,
                    detail=f"El contrato no tiene activo el nivel {nivel} de validación.",
                )
            ...
            for row in candidatos:
                ...
                _sicoe_aplicar_bloqueado_si_nivel_es_maximo(contrato_id, nivel, update, marcar)

                def _upd_hi(regid=rid, upd=update):
                    return (
                        supabase.table("so_registros")
                        .update(upd)
                        .eq("id", regid)
                ...
                if body_in.comentario_data:
                    ...
                    _push_notif_validacion_sicoe_destinatarios(
                        ...
                        contrato_id,
                        rid,
                        f"Validación Nivel {nivel} (masivo): {marcar}",
                        body_in.comentario_data.get("mensaje", "") or "",
                        body_in.comentario_data,
                    )
                try:
                    ...
                    registrar_log(
                        ...
                        "registro",
                        str(rid),
                        {"nivel": nivel, "estado": marcar, "masivo": True, "masivo_filtro": True},
                        valor_anterior=_so_registro_validacion_audit_snapshot(prev_audit),
                        valor_nuevo=_so_registro_validacion_audit_snapshot(after_audit),
                    )
                except Exception:
            ...
            raise HTTPException(status_code=422, detail=f"Nivel masivo no soportado: {nivel}")
...
def validar_sub(contrato_id: int, registro_id: int, body: ValidarSubBody,
    ...
    try:
        ...
        def _get():
            return supabase.table("so_registros")\
                .select(f"nivel2_objeto_pago_sub, contrato_id, {SICOE_SELECT_NIVELES_ESTADO}").eq("id", registro_id)\
                .eq("contrato_id", contrato_id).limit(1).execute().data
        rows = supabase_execute(_get)
        if not rows:
            raise HTTPException(status_code=404, detail="Registro no encontrado.")
        if _registro_nivel3_aprobado(rows[0]):
            raise HTTPException(
                status_code=400,
                detail="El registro está aprobado en el último nivel de validación y no puede modificarse por esta vía.",
            )
...
def _pkid_tabla_rows_to_colores(rows: List[dict]) -> Dict[str, Any]:
    ...
    for row in rows or []:
        ...
        if not pk or pk == "(sin pk)":
            continue
        pref = float(row.get("total_claracore_costo") or row.get("costo_ppto") or 0)
        sap = float(row.get("costo_cobrado") or row.get("costo_sicoe_aprobado") or row.get("costo_sicoe") or 0)
        result[pk] = {
            "cobrado": round(sap, 2),
...
from dashboard_presupuesto_vista import (
    ...
    _drill_item_estado_fields,
    _ppto_claracore_split,
    _ppto_metricas_por_estado,
...
def _build_pkid_tabla_row_from_aggs(
    ...
    return {
        ...
        "revisado": rev_dom,
        "cant_nr": round(pnr["cant"], 2),
        "costo_nr": round(pnr["costo"], 0),
        "cant_p": round(ppd["cant"], 2),
        "costo_p": round(ppd["costo"], 0),
        "cant_r": round(prj["cant"], 2),
        "costo_r": round(prj["costo"], 0),
        "cant_a": round(pap["cant"], 2),
        "costo_a": round(pap["costo"], 0),
        "tiene_ppto_obra_ejecutada": bool(cant_ppto or costo_ppto),
    }


def _sync_pkid_row_financiero_fields(row: dict) -> None:
    """Alias NR|P|R|A + cobrado para misma lógica que tabla ítems."""
    if not isinstance(row, dict):
        return
    row["cant_nr"] = round(float(row.get("cant_nr") or row.get("cant_ppto_estado_no_revisado") or 0), 2)
    row["costo_nr"] = round(float(row.get("costo_nr") or row.get("costo_ppto_estado_no_revisado") or 0), 0)
    row["cant_p"] = round(float(row.get("cant_p") or row.get("cant_ppto_estado_pendiente") or 0), 2)
    row["costo_p"] = round(float(row.get("costo_p") or row.get("costo_ppto_estado_pendiente") or 0), 0)
    row["cant_r"] = round(float(row.get("cant_r") or row.get("cant_ppto_estado_rechazado") or 0), 2)
    row["costo_r"] = round(float(row.get("costo_r") or row.get("costo_ppto_estado_rechazado") or 0), 0)
    row["cant_a"] = round(float(row.get("cant_a") or row.get("cant_ppto_aprobado_n3") or 0), 2)
    row["costo_a"] = round(float(row.get("costo_a") or row.get("costo_ppto_aprobado_n3") or 0), 0)
    cp = float(row.get("cant_ppto") or 0)
    co = float(row.get("costo_ppto") or 0)
    tiene = bool(row.get("tiene_ppto_obra_ejecutada"))
    if not tiene:
        tiene = bool(cp or co or row["cant_a"] or row["costo_a"] or row["cant_nr"] or row["costo_nr"]
                     or row["cant_p"] or row["costo_p"] or row["cant_r"] or row["costo_r"])
    row["tiene_ppto_obra_ejecutada"] = tiene
    row["cant_cobrado"] = round(float(row.get("cant_cobrado") or row.get("cant_sicoe_aprobado") or row.get("cant_sicoe") or 0), 3)
    row["costo_cobrado"] = round(float(row.get("costo_cobrado") or row.get("costo_sicoe_aprobado") or row.get("costo_sicoe") or 0), 2)


def _item_tiene_ppto_obra_ejecutada(
    contrato_id: int,
    capitulo: str,
    item: str,
    current_user,
) -> bool:
    """True si el ítem tiene filas en Presupuesto Obra Ejecutada (misma regla que drill ítems)."""
    cap_raw = (capitulo or "").strip()
    it_key = _dash_norm_item_key_py(item)
    if not cap_raw or not it_key:
        return False
    for r in _ppto_rows_capitulo_tipo(contrato_id, cap_raw, TIPO_OBRA_EJECUTADA, current_user):
        if _dash_norm_item_key_py(r.get("item")) == it_key:
            return True
    return False


def _finalize_pkid_tabla_row(
    row: dict,
    vista: str,
    *,
    item_tiene_ppto_oe: Optional[bool] = None,
) -> None:
    _sync_pkid_row_financiero_fields(row)
    if parse_dash_vista(vista) == DASH_VISTA_OBRA_EJECUTADA:
        _apply_obra_ejecutada_drill_item(row, item_tiene_ppto=item_tiene_ppto_oe)
    else:
        _apply_presupuesto_obra_drill_item(row)


def _enrich_drill_items_meta(
    contrato_id: int,
    capitulo: str,
    rows: List[dict],
    current_user,
) -> List[dict]:
    """Completa descripción y unidad desde presupuesto (Obra / Obra Ejecutada) y SICOE."""
    if not rows:
        return rows
    meta: Dict[str, Dict[str, str]] = {}
    cap_key = _dash_norm_capitulo_key_py(capitulo)
    for tipo in (TIPO_OBRA_EJECUTADA, TIPO_PRESUPUESTO_OBRA):
        for r in _ppto_rows_capitulo_tipo(contrato_id, capitulo, tipo, current_user):
            it = _dash_norm_item_key_py(r.get("item"))
            if not it:
                continue
            slot = meta.setdefault(it, {"descripcion": "", "unidad": ""})
            desc = str(r.get("descripcion") or "").strip()
            und = str(r.get("und") or "").strip()
            if desc and not slot["descripcion"]:
                slot["descripcion"] = desc
            if und and not slot["unidad"]:
                slot["unidad"] = und
    off = 0
    while True:
        def _b(o=off):
            ...
            return apply_sicoe_capitulo_filter(q, supabase, contrato_id, capitulo).order("id").range(o, o + 999).execute().data

        batch = supabase_execute(_b) or []
        for r in batch:
            if _dash_norm_capitulo_key_py(r.get("capitulo")) != cap_key:
                continue
            it = _dash_norm_item_key_py(r.get("item_numero"))
            if not it:
                continue
            slot = meta.setdefault(it, {"descripcion": "", "unidad": ""})
    ...
    for row in rows:
        ...
        m = meta.get(ik) or {}
        if not str(r.get("descripcion") or "").strip() and m.get("descripcion"):
            r["descripcion"] = m["descripcion"]
        if not str(r.get("unidad") or "").strip() and m.get("unidad"):
            r["unidad"] = m["unidad"]
        out.append(r)
    return out


def _hydrate_drill_items_ppto_obra_ejecutada(
    contrato_id: int,
    capitulo: str,
    rows: List[dict],
    current_user,
    ...
    for row in rows:
        ...
        if rows_it:
            ...
            if desc and not str(r.get("descripcion") or "").strip():
                r["descripcion"] = desc
            if unidad and not str(r.get("unidad") or "").strip():
                r["unidad"] = unidad
        else:
            r["tiene_ppto_obra_ejecutada"] = False
        out.append(r)
    return out
...
def _finalize_drill_items_vista(rows: Optional[List[dict]], vista: str) -> List[dict]:
    out: List[dict] = []
    es_oe = parse_dash_vista(vista) == DASH_VISTA_OBRA_EJECUTADA
    for row in rows or []:
        if not isinstance(row, dict):
            continue
        r = dict(row)
        ...
        if es_oe:
            ...
            _apply_presupuesto_obra_drill_item(r)
        out.append(r)
    return out


def _apply_obra_ejecutada_pkid_delta(row: dict, *, item_tiene_ppto_oe: Optional[bool] = None) -> None:
    """Obra Ejecutada PK: misma lógica que ítems, con contexto de ítem para sumas coherentes."""
    _finalize_pkid_tabla_row(row, DASH_VISTA_OBRA_EJECUTADA, item_tiene_ppto_oe=item_tiene_ppto_oe)
...
def _normalize_pkid_tabla_payload(
    ...
    *,
    ...
    if not isinstance(rows, list):
        rows = []
    if (
        item_tiene_ppto_oe is None
        and parse_dash_vista(vista) == DASH_VISTA_OBRA_EJECUTADA
        and contrato_id
        and (item or "").strip()
    ):
    ...
    return {
        ...
        "meta": {
            "item_tiene_ppto_obra_ejecutada": item_tiene_ppto_oe,
            "pk_cc_coherente_item": parse_dash_vista(vista) == DASH_VISTA_OBRA_EJECUTADA,
        },
    }
...
def _merge_drill_rpc_sicoe_vista_ppto(rpc_items: List[dict], ppto_items: List[dict]) -> List[dict]:
    ...
    _fin_fields = (
        "unidad",
        "cant_nr", "costo_nr", "cant_p", "costo_p", "cant_r", "costo_r", "cant_a", "costo_a",
        "total_claracore_cant", "total_claracore_costo", "cant_cobrado", "costo_cobrado",
        "delta_cant", "delta_costo",
    )
    ...
    for ik in keys:
        ...
        row = {
            ...
            "cant_cobrado": round(float(r.get("cant_sicoe_aprobado") or 0), 3),
            "costo_cobrado": round(cob, 2),
        }
        for f in _fin_fields:
            if f in p and p.get(f) is not None:
                row[f] = p[f]
        if p:
            row["tiene_ppto_obra_ejecutada"] = bool(p.get("tiene_ppto_obra_ejecutada", True))
        out.append(row)
    return out
    ...
    while True:
        ...
        for reg in batch or []:
            ...
            if it_norm and _dash_norm_item_key_py(reg.get("item_numero")) != it_norm:
                continue
            if not sicoe_registro_en_vista(
                reg,
                None if parse_dash_vista(vista) == DASH_VISTA_OBRA_EJECUTADA else allowed_keys,
            ):
                continue
    ...
    for merged in ws.merged_cells.ranges:
        if (
            ...
            for c in range(merged.min_col, merged.max_col + 1):
                ...
                total += float(ws.column_dimensions[letter].width or 12)
            return total
    letter = get_column_letter(col)
    return float(ws.column_dimensions[letter].width or 12)


def _xlsx_estimate_row_height_pt(
    ws,
    row: int,
    cols: Iterable[int],
    *,
    ...
    """Altura mínima o mayor si el texto envuelto lo requiere (wrap_text)."""
    max_lines = 1
    for col in cols:
        cell = ws.cell(row=row, column=col)
        if cell.value is None:
            continue
        eff_w = _xlsx_merged_span_width(ws, row, col)
        chars = max(int(eff_w * 0.85), 8)
        lines = 0
        for part in str(cell.value).replace("\r", "").split("\n"):
            pl = len(part) or 1
            lines += max(1, (pl + chars - 1) // chars)
        max_lines = max(max_lines, lines)
    line_h = font_size * 1.45
    return max(min_height, max_lines * line_h + 8)
...
def _fetch_export_core_by_item(
    ...
    *,
    ...
    def _one_item(it: str) -> Tuple[str, Dict[str, Any]]:
        ...
        return it, _normalize_pkid_tabla_payload(
            raw,
            contrato_id,
            alias_map=alias_map,
            vista=vista,
...
def _gerencial_capitulos_data_obra_ejecutada(
    ...
    out: List[Dict[str, Any]] = []
    for ck, d in cap_agg.items():
        cc = float(d["claracore"])
        cob = float(d["cobrado"])
        out.append(
            {
                "capitulo": d.get("display") or ck,
                "claracore": round(cc, 0),
                "cobrado": round(cob, 0),
                "delta": round(cc - cob, 0),
                "aprobado": round(float(d["aprobado"]), 0),
                "pendiente": round(float(d["pendiente"]), 0),
                "rechazado": round(float(d["rechazado"]), 0),
                "no_revisado": round(float(d["no_revisado"]), 0),
            }
        )
    out.sort(
        key=lambda r: (
...
def _require_dashboard_export(current_user, contrato_id: int) -> None:
    """Matriz admin: función Dashboard · acción exportar (Desarrollador siempre)."""
    if _es_desarrollador(current_user):
        return
    if _cargo_tiene_permiso_funcion(current_user, "Dashboard", "exportar", contrato_id):
        return
    raise HTTPException(status_code=403, detail="No tiene permiso para exportar informes del Dashboard")


def _xlsx_fit_image_to_cell(img, row_height_pt: float, col_width_chars: float, pad_pt: float = 3) -> None:
    """Escala logo para llenar la altura de la fila (sin superar el ancho de columna)."""
    max_h = max(int((row_height_pt - pad_pt) * 96 / 72), 20)
    max_w = max(int(col_width_chars * 7.2), 60)
    h = float(img.height or 1)
    w = float(img.width or 1)
    scale = min(max_h / h, max_w / w)
    img.height = max(1, int(h * scale))
    img.width = max(1, int(w * scale))


def _xlsx_fetch_image_bytes(url: Optional[str]) -> Optional[bytes]:
    """Logo/imagen del contrato (URL pública, Azure, data-URI base64)."""
    u = (url or "").strip()
    if not u:
        return None
    ...
    try:
        ...
        r = requests.get(
            u,
            timeout=25,
            headers={"User-Agent": "ClaraCore-Export/1.0"},
        )
        r.raise_for_status()
        data = r.content
        ct = (r.headers.get("content-type") or "").lower()
        if data and len(data) > 80 and ("image/" in ct or not ct):
            return data
    except Exception:
        pass
    return None
...
def _contrato_presupuesto_version_label(contrato_id: int) -> str:
    try:
        rows = (
            supabase.table("presupuesto_versiones")
            .select("nombre, codigo, es_vigente")
            .eq("contrato_id", int(contrato_id))
            .eq("es_vigente", True)
            .limit(1)
            .execute()
        ...
        if rows:
            r0 = rows[0]
            lbl = (r0.get("nombre") or r0.get("codigo") or "Vigente") or "Vigente"
...
def _build_dashboard_gerencial_xlsx(
    ...
    for j, h in enumerate(hdr, start=1):
        ...
        cell.alignment = al_center if j == 1 else al_right
        cell.border = border_tbl
    ws.row_dimensions[hr].height = _xlsx_estimate_row_height_pt(
        ws, hr, range(1, ncols + 1), min_height=_ROW_H_INFO, font_size=11
    )
    ...
    for idx, r in enumerate(rows):
        ...
        cap_cell.font = Font(bold=True, size=10, color=_CC_XLSX_DARK)
        cc_v = round(float(r.get("claracore") or 0), 0)
        co_v = round(float(r.get("cobrado") or 0), 0)
        tot_cl += cc_v
        tot_co += co_v
        cc_cell = ws.cell(row=ri, column=2, value=cc_v)
        cc_cell.number_format = _XLSX_FMT_COP
        cc_cell.alignment = al_right
        cc_cell.border = border_tbl
        cc_cell.font = Font(bold=True, color=_CC_XLSX_DARK)
        co_cell = ws.cell(row=ri, column=3, value=co_v)
        co_cell.number_format = _XLSX_FMT_COP
        co_cell.alignment = al_right
        co_cell.border = border_tbl
        co_cell.font = Font(bold=True, color=_CC_XLSX_DARK)
        d_cell = ws.cell(row=ri, column=4, value=f"=B{ri}-C{ri}")
        d_cell.number_format = _XLSX_FMT_COP
        d_cell.alignment = al_right
        d_cell.border = border_tbl
        delta_v = cc_v - co_v
        d_cell.font = Font(
            bold=True,
            color="DC2626" if delta_v < -0.5 else _CC_XLSX_DARK,
    ...
    for j in range(2, ncols + 1):
        ...
        if j == 4:
            ...
            defaults = {2: tot_cl, 3: tot_co}
            cell = ws.cell(row=ri, column=j, value=round(defaults.get(j, 0), 0))
        cell.number_format = _XLSX_FMT_COP
        cell.alignment = al_right
        cell.font = Font(bold=True, color="FFFFFF")
        cell.border = border_tbl
    for j in range(1, ncols + 1):
        ws.cell(row=ri, column=j).fill = fill_tot
    ws.row_dimensions[ri].height = _xlsx_estimate_row_height_pt(
        ws, ri, [1], min_height=_ROW_H_INFO, font_size=10
    )
    ...
    fn.font = Font(size=8, color="64748B", italic=True)
    fn.alignment = al_center
    ws.row_dimensions[footer_note_row].height = _xlsx_estimate_row_height_pt(
        ws, footer_note_row, [1], min_height=_ROW_H_INFO, font_size=8
    )

    ws.print_area = f"A1:D{footer_note_row}"
    ...
    try:
        ...
        footer_item = HeaderFooterItem()
        footer_item.text = (
            "&LClaraCore&CInforme gerencial - Uso confidencial del contrato&RGenerado con ClaraCore"
        )
        ws.oddFooter = footer_item
    except Exception:
        pass
    ...
    bio = io.BytesIO()
    wb.save(bio)
    fn_name = f"ClaraCore_Gerencial_{datetime.now(pytz.timezone('America/Bogota')).strftime('%Y-%m-%d')}.xlsx"
...
def dashboard_export_capitulo_download_obra(
    ...
    _require_contract_access(current_user, contrato_id)
    _require_dashboard_export(current_user, contrato_id)
    try:
...
@app.get("/sicoe-obra/{contrato_id}/dashboard-capitulos-financiero")
...
def dashboard_export_gerencial_download_obra(
    ...
    """Informe gerencial (todos los capítulos, solo financiero) alineado con tabla dashboard."""
    _require_contract_access(current_user, contrato_id)
    _require_dashboard_export(current_user, contrato_id)
    try:
    ...
    try:
        ...
        def _fmt_sicoe(r: dict) -> dict:
            return {
                ...
                "nivel3_estado": _matriz_validacion_norm_estado_nivel_final(r, contrato_id),
            }
...
def solicitar_reversion(contrato_id: int, registro_id: int, body: SolicitarReversionBody,
    ...
    try:
        ...
        def _get():
            return supabase.table("so_registros")\
                .select(f"{SICOE_SELECT_NIVELES_ESTADO},bloqueado,contrato_id").eq("id", registro_id)\
                .eq("contrato_id", contrato_id).limit(1).execute().data
        rows = supabase_execute(_get)
        if not rows:
            raise HTTPException(status_code=404, detail="Registro no encontrado.")
...
def validar_masivo_nivel3(contrato_id: int, reporte_id: int, body: ValidarMasivoNivel3Body,
    ...
    try:
        ...
        for reg_id in ids_all[:SICOE_MASIVO_MAX_REGISTROS]:
            ...
            _sicoe_aplicar_bloqueado_si_nivel_es_maximo(contrato_id, 3, update, body.estado)
    ...
    try:
        ...
        while True:
            ...
            for row in rows:
                revisados += 1
                ref_raw = None
                if _matriz_validacion_norm_estado_nivel_final(row, contrato_id) == "Aprobado":
                    mxn = _get_nivel_numero_maximo_contrato(contrato_id)
                    ref_raw = (
                        row.get(f"nivel{mxn}_fecha")
                        or row.get("nivel3_fecha")
                        or row.get("nivel2_fecha")
                    )
