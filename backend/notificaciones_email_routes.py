...
# Eliminar _jwt_user_from_request, _destinatario_prueba_temp_desarrollador,
...
def _destinatario_prueba_temp_desarrollador(current_user: Optional[dict]) -> tuple[str, str]:
    """Solo JWT de desarrollador; correo siempre el del usuario autenticado (sin parámetros externos)."""
    from notificaciones_email_service import _usuario_display_name

    if not current_user:
        raise HTTPException(
            status_code=403,
            detail="Requiere Authorization Bearer de un usuario desarrollador",
        )
    import main as m
...
def temp_prueba_resumen_jornada(
    ...
    Solo envía al correo del desarrollador autenticado (JWT); no acepta destino externo.

    Auth: únicamente Authorization Bearer de usuario desarrollador.
    """
    import main as m
    from notificaciones_email_service import build_runner_from_main

    current_user = _jwt_user_from_request(request)
    dest_email, dest_nombre = _destinatario_prueba_temp_desarrollador(current_user)
    runner = build_runner_from_main(m)
    try:
