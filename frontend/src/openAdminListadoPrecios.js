...
/**
 * Abre Panel admin → Gestión de Usuarios, opcionalmente enfocando un usuario.
 * @param {object} [options]
 * @param {number|string} [options.usuarioId]
 */
export function openAdminUsuarios(options = {}) {
  const uid = options.usuarioId != null ? String(options.usuarioId).trim() : ''
  const payload = { tab: 'usuarios', ...(uid ? { usuarioId: uid } : {}) }
  try {
    sessionStorage.setItem('cc_admin_nav', JSON.stringify(payload))
  } catch {
    /* ignore */
