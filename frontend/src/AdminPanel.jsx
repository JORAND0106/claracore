...
// ─── Panel Soporte (solo Desarrollador) ─────────────────────────────────────
...
function SeccionSoporte({ call, theme }) {
  const col = C(theme);
  const [data, setData] = useState(null);
  ...
  const cargar = useCallback(async () => {
    setErr(null);
    try {
      const j = await call("GET", `/admin/soporte?filtro=${encodeURIComponent(filtro)}`);
      setData(j);
    } catch (e) {
      setErr(e?.message || String(e));
      setData(null);
  ...
  useEffect(() => {
    const t = setInterval(() => { void cargar(); }, 20000);
  ...
  const marcar = async (id, accion) => {
    setAccionId(id);
    try {
      await call("PUT", `/admin/soporte/${id}/gestionar`, { accion });
      await cargar();
    } catch (e) {
      alert(e?.message || "No se pudo actualizar el reporte");
    } finally {
      setAccionId(null);
    }
  };

  const kpis = data?.kpis || { pendientes: 0, errores_hoy: 0, sugerencias: 0, gestionados: 0 };
  ...
  const kpiCard = (label, value, accent) => (
    ...
    >
      <div style={{ fontSize: 11, fontWeight: 700, color: col.textMuted, textTransform: "uppercase", letterSpacing: "0.04em" }}>
        {label}
      </div>
      <div style={{ fontSize: 28, fontWeight: 800, color: accent, marginTop: 4, fontVariantNumeric: "tabular-nums" }}>
        {value}
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 18 }}>
        {kpiCard("Pendientes", kpis.pendientes, "#EA580C")}
        {kpiCard("Errores hoy", kpis.errores_hoy, "#DC2626")}
        {kpiCard("Sugerencias", kpis.sugerencias, "#CA8A04")}
        {kpiCard("Gestionados", kpis.gestionados, "#16A34A")}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, marginBottom: 16 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? `${col.primary}22` : col.bgCard,
                  color: active ? col.primary : col.textMuted,
                }}
              >
                {f.label}
              </button>
        ...
        <button type="button" style={S.btn("ghost")} onClick={() => { setLoading(true); void cargar(); }} disabled={loading}>
          {loading ? "Actualizando…" : "↻ Actualizar"}
        </button>
      </div>

      {loading && !reportes.length ? (
        <div style={{ color: col.textMuted, padding: 24, textAlign: "center" }}>Cargando reportes…</div>
      ) : !reportes.length ? (
        <div style={{ color: col.textMuted, padding: 24, textAlign: "center" }}>No hay reportes en este filtro.</div>
      ) : (
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 15, color: col.text, marginBottom: 6 }}>
                          {r.asunto || "Sin asunto"}
                        </div>
                        <div style={{ marginBottom: 8 }}>
                          {badge(
                            esError ? "Error" : esSug ? "Sugerencia" : "Soporte",
                            esError ? "#FEE2E2" : esSug ? "#FEF9C3" : "#E0F2FE",
                            esError ? "#991B1B" : esSug ? "#854D0E" : "#0369A1",
                          )}
                          {r.urgencia && badge(r.urgencia, "#FFF7ED", "#9A3412")}
                          {gestionado && badge(
                        ...
                        <div style={{ fontSize: 12, color: col.textMuted, marginBottom: 8 }}>
                          {r.remitente_nombre || "Usuario"}
                          {r.modulo ? ` · ${r.modulo}` : ""}
                          {r.contrato_id != null ? ` · Contrato ${r.contrato_id}` : ""}
                          {" · "}
                          {tiempoRelativo(r.created_at)}
                        </div>
                        <div style={{ fontSize: 13, color: col.text, lineHeight: 1.45 }}>
                          {r.descripcion_resumen || "—"}
                        </div>
                        {gestionado && (
                          <div style={{ fontSize: 11, color: col.textMuted, marginTop: 8 }}>
                            {r.soporte_estado === "anotado" ? "💡 Anotado" : "✅ Gestionado"}
                            {r.soporte_gestionado_at ? ` · ${formatFechaLogBogota(r.soporte_gestionado_at)}` : ""}
                            {r.soporte_gestionado_por_nombre ? ` · ${r.soporte_gestionado_por_nombre}` : ""}
                            {r.soporte_gestion_origen ? ` (${r.soporte_gestion_origen})` : ""}
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {!gestionado && (esError || esSug) && (
                    ...
                    >
                      {busy ? "…" : esError ? "✅ Marcar gestionado" : "💡 Marcar anotado"}
                    </button>
                  )}
                </div>
              </div>
...
const ADMIN_PANEL_TABS = [
  ...
  { id: "soporte", label: "Soporte", soloDeveloper: true },
];
...
export default function AdminPanel({ user, token, onClose, activeTheme, t: tProp }) {
  ...
  const TITULOS = {
    ...
    soporte: {
      title: "Soporte",
      sub: "Reportes de error y sugerencias de mejora enviados por los usuarios (solo Desarrollador).",
    },
  };
        ...
        <div style={S.content}>
          ...
          />}
            ...
            {tab === "inicio"           && (
              ...
              {tab === "soporte" && <SeccionSoporte call={call} theme={activeTheme} />}
          </div>
        </div>
      </div>
